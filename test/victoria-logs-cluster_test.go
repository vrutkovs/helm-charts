...
func TestVictoriaLogsClusterInstallDefault(t *testing.T) {
	...
	k8s.WaitUntilServiceAvailable(t, kubectlOptions, vlInsertName, retries, resourceWaitTimeout)
	...
	k8s.WaitUntilServiceAvailable(t, kubectlOptions, vlSelectName, retries, resourceWaitTimeout)

	// Verify vlselect Service was created and is available
	vlStorage := fmt.Sprintf("%s-victoria-logs-cluster-vlstorage", releaseName)
	k8s.WaitUntilServiceAvailable(t, kubectlOptions, vlStorage, retries, resourceWaitTimeout)
}
